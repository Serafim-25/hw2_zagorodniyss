# Импортируем необходимые библиотеки из
# ClearML для работы с пайплайнами и задачами
from clearml import PipelineDecorator, Task

# Импортируем библиотеку subprocess для
# выполнения команд в операционной системе
import subprocess

# Импортируем библиотеку os для работы с операционной системой,
# например, для управления путями или файловыми системами
import os


# Декоратор PipelineDecorator для компонента dvc_pull,
# который не использует кеширование
# и выполняется в очереди "default". Этот компонент
# будет отвечать за скачивание данных и зависимостей.
@PipelineDecorator.component(cache=False, execution_queue="default")
def dvc_pull():
    # Выполняем команду "dvc pull", которая обновляет
    # локальную копию данных и моделей
    subprocess.run(["dvc", "pull"], check=True)


# Декоратор PipelineDecorator для компонента download_data,
# который не использует кеширование
# и выполняется в очереди "default". Этот компонент
# запускает скрипт для загрузки данных.
@PipelineDecorator.component(cache=False, execution_queue="default")
def download_data():
...
# Декоратор PipelineDecorator для компонента process_data,
# который не использует кеширование
# и выполняется в очереди "default". Этот компонент
# запускает скрипт для обработки данных.
@PipelineDecorator.component(cache=False, execution_queue="default")
def process_data():
    # Запускаем Python-скрипт "process_data.py"
    # для обработки загруженных данных
    subprocess.run(["python", "scripts/process_data.py"], check=True)


# Декоратор PipelineDecorator для компонента train_model_1,
# который выполняется в очереди "default".
...
def train_model_1():
    try:
        ...
        if current_task:
            # Извлекаем ID текущей задачи и выводим
            # его в консоль для логирования
            task_id = current_task.id
            print(f"Идентификатор текущей задачи в train_model_1: {task_id}")

            # Инициализируем новую задачу в проекте
            # "my_mlops_project" для обучения модели
            new_task = Task.init(
                project_name="my_mlops_project",
                task_name="train_with_clearml_1"
            )

            # Запускаем задачу для выполнения удаленно на очереди 'default'
            new_task.execute_remotely(
                queue_name="default"
            )  # Запуск задачи на удаленной машине/агенте
            ...
            # Если текущая задача не существует,
            # выводим ошибку и выбрасываем исключение
            print("Current task is None in train_model_1")
            raise ValueError("Current task is None in train_model_1")
    except Exception as e:
        # Ловим исключения и выводим ошибку, если что-то
        # пошло не так в процессе обучения
        print(f"Ошибка в компоненте train_model_1: {e}")
        raise  # Пробрасываем исключение дальше


# Декоратор PipelineDecorator для компонента train_model_2,
# который выполняется в очереди "default".
...
def train_model_2():
    try:
        ...
        if current_task:
            ...
            # Если текущая задача не существует, выводим
            # ошибку и выбрасываем исключение
...
# Декоратор PipelineDecorator для компонента dvc_repro,
# который выполняет команду DVC "repro".
# Эта команда запускает воспроизведение состояния данных в DVC,
# восстанавливая нужные версии данных и зависимостей.
@PipelineDecorator.component(cache=False, execution_queue="default")
def dvc_repro():
    # Выполняем команду "dvc repro" для воспроизведения
    # данных и состояний в DVC
    subprocess.run(["dvc", "repro"], check=True)


# Декоратор PipelineDecorator для компонента dvc_push,
# который выполняет команду DVC "push".
# Эта команда отправляет изменения в репозиторий DVC,
# обновляя удаленные хранилища данными.
@PipelineDecorator.component(cache=False, execution_queue="default")
def dvc_push():
...
# Основной блок для выполнения пайплайна
# на локальной машине, полезно для отладки.
# Для масштабирования, нужно будет закомментировать следующую
# строку и настроить очередь 'services' на ClearML.
if __name__ == "__main__":
    # Запуск пайплайна локально, используем для тестирования на текущей машине
    # Для масштабирования можно закомментировать
    # эту строку и использовать очередь 'services' с агентом ClearML
