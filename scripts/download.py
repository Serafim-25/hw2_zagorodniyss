...
from sklearn.datasets import load_iris  # Импортируем функцию для загрузки набора данных iris_data из sklearn
...
def download_data(output_path):
    ...
    iris_df = pd.DataFrame(data=iris_data.data, columns=iris_data.feature_names)  # Создаем DataFrame с данными признаков (features)

    # Добавляем столбец 'species' с целевыми метками (классами)
    iris_df['species'] = iris_data.target  # Добавляем столбец 'species' с целевыми метками, представляющими виды
...
download_data('data/iris_dataset.csv')  # Вызываем функцию для загрузки данных и их сохранения
