...
from sklearn.linear_model import LogisticRegression  # Импортируем логистическую регрессию для обучения модели
from sklearn.model_selection import train_test_split  # Импортируем функцию для разделения данных на обучающие и тестовые
from sklearn.metrics import accuracy_score  # Импортируем метрику точности для оценки модели
from sklearn.datasets import load_iris  # Импортируем набор данных Iris для примера
...
def main():
    ...
    task_name = task.name if task else "local"  # Если задача
                                                # не найдена, используем "local" как имя задачи
    ...
    with mlflow.start_run(run_name=f"{task_name}-{args.model_type}"):
         ...
         # Инициализируем модель логистической регрессии с выбранными параметрами
